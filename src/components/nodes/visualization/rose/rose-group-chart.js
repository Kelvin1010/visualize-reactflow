...
function RoseGroupChart({ onCallback, id }) {
...
export default RoseGroupChart

function Sidebar({ onDragStart }) {
    return (
      <div className="dndnode" onDragStart={(event) => onDragStart(event, "rose-group-chart")} draggable>
        Biểu đồ rose nhóm
      </div>
    );
}
...
export function RoseGroupChartWrapper(props) {
    return (
      <NodeContainer {...props} label="Biểu đồ rose nhóm" isLeftHandle className="chart-container">
        <RoseGroupChart />
      </NodeContainer>
    );
}
  
RoseGroupChartWrapper.Sidebar = Sidebar;